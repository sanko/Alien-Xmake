...
class Alien::Xmake::Builder {
    ...
    # Configuration
    field $LastRelease : param : reader //= 'v3.0.6';
    field $force  : param  //= 0;
    field $meta   : reader //= CPAN::Meta->load_file('META.json');
    field $action : param  //= 'build';
    field $TargetFile = 'lib/Alien/xmake/ConfigData.pm';
...
%s->new( @ARGV && $ARGV[0] =~ /\A\w+\z/ ? ( action => shift @ARGV ) : (),
    ...
    method _command_exists ($cmd) {
        my $sep = ( $^O eq 'MSWin32' ) ? ';' : ':';
        for my $dir ( split /$sep/, $ENV{PATH} ) {
            my $p    = path($dir);
            my $exts = ( $^O eq 'MSWin32' ) ? [qw[.exe .cmd .bat .com]] : [''];
            for my $ext (@$exts) {
                return 1 if -x $p->child("$cmd$ext");
            }
        }
        return 0;
    }
    ...
    method _resolve_xmake ( ) {
        ...
        unless ($force) {
            ...
            if ($sys_path) {
                ...
                if ( $self->_version_cmp( $ver, $LastRelease ) >= 0 ) {
                    say "Found suitable system xmake: $sys_path ($ver)";
                    return { install_type => 'system', version => $ver, bin => "$sys_path" };
                }
                say "System xmake found ($ver) but is older than required ($LastRelease).";
            }
        }
        ...
        if ($existing) {
            ...
            if ( $self->_version_cmp( $ex_ver, $LastRelease ) >= 0 ) {
    ...
    method _write_config_data ($data) {
        my $dest = path('blib')->child($TargetFile);
        $dest->parent->mkpath;
    ...
    method _install_windows ($installdir) {
        ...
        my $filename   = "xmake-$LastRelease.$winarch.exe";
        my $url        = "https://github.com/xmake-io/xmake/releases/download/$LastRelease/$filename";
    ...
    method _install_unix ($installdir) {
        ...
        unless ( $self->_test_tools() ) {
            say "Missing build dependencies (git, make, or compiler).";

            # SAFEGUARD: Do not auto-install system tools unless requested.
            if ( $ENV{ALIEN_INSTALL_SYSTEM_TOOLS} ) {
                ...
                say "Set ENV ALIEN_INSTALL_SYSTEM_TOOLS=1 to attempt auto-installation.";
                $self->_raise_dep_error();
            }
        }
        my $version  = $LastRelease;
        ...
        say "Building xmake...";
        my $make = $self->_run_cmd("gmake --version >/dev/null 2>&1") ? "gmake" : "make";

        if ( -f "configure" ) {
            system("./configure") == 0  or die "Configure failed";
            system( $make, "-j4" ) == 0 or die "Make failed";
            say "Installing to $installdir...";

            # Using specific ENV variable to control make install prefix behavior usually handles recursive mkdir
            system( $make, "install", "PREFIX=$installdir" ) == 0 or die "Install failed";
        }
        else {
            system( $make, "build",   "-j4" ) == 0                or die "Make build failed";
            system( $make, "install", "prefix=$installdir" ) == 0 or die "Make install failed";
        }
        chdir $cwd;
    }
    ...
    method _download_file ( $url, $dest ) {
        ...
        # Try HTTP::Tinyand IO::Socket::SSL
    ...
    method _test_tools ( ) {
        my $make_cmd = $self->_run_cmd("gmake --version >/dev/null 2>&1") ? "gmake" : "make";
        return 0 unless $self->_run_cmd("git --version >/dev/null 2>&1");
        return 0 unless $self->_run_cmd("$make_cmd --version >/dev/null 2>&1");
        my $prog = "#include <stdio.h>\nint main(){return 0;}";
        my @compilers
            = ( [ 'cc', '-xc', '-', '-o', '/dev/null' ], [ 'gcc', '-xc', '-', '-o', '/dev/null' ], [ 'clang', '-xc', '-', '-o', '/dev/null' ] );
        for my $cmd_ref (@compilers) {
            my @cmd = @$cmd_ref;
            next unless $self->_command_exists( $cmd[0] );
            my $cmd_str = join( ' ', @cmd );
    ...
    method _install_tools ($sudo) {
        my @installers = (
            ...
            [ 'pkg list-installed',     'pkg install -y git getconf build-essential readline' ],
    ...
    Or set ALIEN_INSTALL_SYSTEM_TOOLS=1 to allow auto-installation.
    MSG
    }
