package builder::xmake {
    ...
    #
    my $version = '3.0.6';                            # Target install version
    ...
    sub gather_info {
        ...
        my ( $stdout, $stderr, $exit ) = capture { system $xmake, '--version' };
        my ($ver) = $stdout =~ m[xmake (v.+?), A cross-platform build utility based on Lua];
...
#~ exit 1;
    ...
    sub build_from_source {
        ...
        if ( !defined $make || !defined $compiler ) {
            ...
            $s->log_info( 'You could also install a prebuilt version of xmake with ' . $prebuilt . "\n" ) if defined $prebuilt;
        }
